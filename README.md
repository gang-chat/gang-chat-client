...
音乐盒是房间语音频道中的服务端权威协同播放功能。播放状态、队列、当前歌曲、进度和活动来源由服务端同步，客户端负责操作、展示、本地试听和缓存。

#### 搜索与点歌

- 当前开放网易云和哔哩哔哩搜索源。
- QQ 音乐路由实现暂时保留，但客户端入口当前禁用。
- 搜索结果显示完整歌曲标题和作者/歌手；哔哩哔哩来源使用“作者”字段并显示 BV 号等详情。
- 点击加号加入点歌队列；相同具体链接已在队列中时给出提示，不重复添加。
- 下载/准备歌曲时，歌曲图标位置显示加载状态。

#### 点歌队列与播放控制

- 点歌队列是每个房间的临时播放来源。
- 支持播放、暂停、上一首、下一首和播放模式切换。
- 支持顺序播放、单曲循环、列表循环和随机播放等服务端模式。
- 未在播放时点击播放，会从当前正在查看的歌单第一首开始。
- “优先播放”会立即播放所选点歌，并把它放到刚才播放歌曲之后，维持后续队列关系。
- 可以删除单项或清空点歌队列；空队列时清空操作禁用。
- 正在播放、暂停、下载、失败和待处理状态分别展示，不用每秒重建按钮造成闪烁。
- 打开音乐面板时自动把正在播放的歌曲滚动到可见位置；已经靠近顶部或底部时不强制居中。
- 长歌曲名使用横向往返展示，确保完整内容可以被看到。

#### 浏览来源

- 音乐盒标题显示当前“所视歌单”，而不是始终显示当前正在播放的歌单。
- 展开后可搜索全部来源，并按我的歌单/房间歌单筛选；再次点击筛选可取消。
- 当前实际播放来源和点歌队列也遵循筛选条件。
- 当前实际播放的来源在右侧显示波形图标；停止状态不会错误标记“正在播放”。
- 选择歌单只切换查看内容，不会立刻改变播放；点击该歌单中的具体歌曲或播放按钮后才激活来源。
- 点击面板外部才收起来源列表，列表内部空白区域不会误触收起。
- 个人歌单或房间歌单为空时，在有权限的情况下显示“新建第一个歌单”并跳转到对应设置页。
...
- 哔哩哔哩 BV 号等来源详情。
- 正在播放、优先播放、已在队列中、点歌队列、试听/取消试听和添加到歌单等上下文操作。
...
- 确认编辑请求实际成功并出现成功提示。
- 重新展开音乐盒来源列表触发刷新。
- 区分“当前浏览歌单”和“已经激活的播放快照”：编辑保存歌单不会修改正在播放中的不可变快照，这是预期行为。
