...
房间歌单使用 `playlist_scope: "room"` 并提供 `source_room_id`。服务端必须重新验证私人歌单归属或源房间访问权，从权威歌单读取歌曲，再把附件替换成 `type: music_track` 与完整 `track` 的不可变快照，并把正文规范化为 `[歌曲] 歌曲名`。源歌单或歌曲之后被删除不影响历史消息。嵌套歌曲消息复用已有歌曲名片：支持本地试听、取消试听和添加到当前用户已有个人歌单。

消息记录分类 `music` 同时匹配 `playlist` 和 `music_track`，客户端显示为“音乐”；其他文件、语音、图片和系统分类规则不变。

右键复制已分享的歌单/歌曲后，在 Gang Chat 输入区粘贴会生成完整组件预览，并提供与引用卡片一致的 `×` 移除入口。转发时附件只提交原消息引用：
