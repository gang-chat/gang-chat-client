...
右键复制已分享的歌单/歌曲后，在 Gang Chat 输入区粘贴会生成完整组件预览，并提供与引用卡片一致的 `×` 移除入口。组件识别应兼容 Windows CRLF、macOS/Linux LF 和剪贴板桥接器追加的末尾换行；只有剪贴板正文仍与最近复制组件一致时才拦截普通文字粘贴。转发时附件只提交原消息引用：
