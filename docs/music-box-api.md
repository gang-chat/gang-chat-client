...
客户端队列标题行显示当前查看来源，首次进入时默认查看当前活动来源；之后查看来源与播放来源彼此
独立，不会因播放状态刷新而强制跳回。点击摘要展开统一歌单浏览器：当前活动歌单（若有）
置顶，点歌队列第二，其后列出个人与房间歌单。“我的歌单”“房间歌单”筛选可再次点击取消，
且不影响两个置顶入口。点击歌单会立即收起展开列表，标题切换为该查看来源，并直接在原内容区
显示歌曲，不进入带返回按钮的下一级页面，也不调用激活接口；只有点击歌曲行的播放动作才携带
