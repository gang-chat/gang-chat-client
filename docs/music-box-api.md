...
个人歌单或当前用户有权管理的房间歌单中的单首歌曲通过普通房间消息接口分享。客户端只提交来源引用，不提交可信歌曲详情：
...
房间歌单使用 `playlist_scope: "room"` 并提供 `source_room_id`。服务端必须重新验证私人歌单归属或源房间访问权，从权威歌单读取歌曲，再把附件替换成 `type: music_track` 与完整 `track` 的不可变快照，并把正文规范化为 `[歌曲] 歌曲名`。源歌单或歌曲之后被删除不影响历史消息。嵌套歌曲消息复用已有歌曲名片：支持本地试听、取消试听，并在“添加到歌单”中同时列出当前用户的全部个人歌单和消息所在房间的全部房间歌单；提交时必须根据目标作用域分别调用个人或房间歌单接口，不能把房间歌单 ID 发往个人接口。

消息记录分类 `music` 同时匹配 `playlist` 和 `music_track`，客户端显示为“音乐”；其他文件、语音、图片和系统分类规则不变。

右键复制已分享的歌单/歌曲后，在 Gang Chat 输入区粘贴会生成完整组件预览，并提供与引用卡片一致的 `×` 移除入口。剪贴板对外仍显示完整可读文本；客户端同时追加不可见、带版本号的结构化信封，至少包含来源消息 ID、房间 ID、附件快照与发送者快照。粘贴时优先校验并解析信封，因而跨焦点、跨页面和进程内缓存失效后仍能恢复组件；没有合法信封的外部文字必须按普通文字处理。组件识别仍兼容 Windows CRLF、macOS/Linux LF 和剪贴板桥接器追加的末尾换行。转发时附件只提交原消息引用：
...
服务端必须验证发送者仍可访问原消息所在房间，再复制原消息中已存储的不可变快照；客户端不能回传或伪造快照字段。
...
客户端音乐盒标题行显示当前活动来源标签；面板主体是一个整体滚动的扁平列表，依次为始终展开的
“点歌队列”和默认折叠的“房间歌单 / 我的歌单”两个分组。点歌队列标题行提供搜索开关、“播放”与清空按钮：
搜索开关展开一行搜索输入框与来源选择，输入非空时队列行被搜索结果替换且两个歌单分组隐藏，收起搜索时
清空搜索框。分组在第一次展开时加载歌单列表；点击歌单行在原地展开其歌曲，只读取模板或活动快照，不调用
激活接口。展开状态由用户决定，不会因播放状态刷新而改写；点击标题行的来源标签把列表滚动到拥有该来源的
区块，已保存歌单会展开所属分组并原地展开该歌单。歌单行没有整单播放；展开歌单中歌曲行唯一的
动作是携带 `start_item_id` 激活并起播（选中歌单里的一首歌即切换当前来源到该歌单），歌单分组内
不提供点歌或添加到歌单的行内动作。正在播放已保存歌单时，点歌队列每一行提供“从这首歌开始播放
点歌队列”，以 `temporary` 与该行 `start_item_id` 激活；点歌队列为当前来源时队列行提供行内“优先播放”与
“从点歌队列删除”按钮，删除调用 `DELETE /queue/:item_id`。`clear_temporary_playlist` 由点歌队列
标题行的清空按钮经二次确认后调用，并继续保留给其他受控入口及兼容客户端。
